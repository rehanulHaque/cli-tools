...
program.command('add <title> <description> <start_date> <end_date>')
    ...
    .action(async (title, description, start_date, end_date) => {
    try {
        const file = await fs.readFile(DATA_FILE, 'utf-8');
        const data = JSON.parse(file);
        const projects = data.projects || [];
...
program.command('delete <id>')
    ...
    try {
        ...
        const projects = data.projects || [];
        const pid = parseInt(id, 10);
        const idx = projects.findIndex((project) => project.id === pid);
        if (idx === -1) {
            console.error(chalk.red(`Project with id ${id} not found`));
            process.exitCode = 2;
            return;
...
program
    ...
    try {
        ...
        if (options.start_date !== undefined)
            project.start_date = options.start_date;
        if (options.end_date !== undefined)
            project.end_date = options.end_date;
